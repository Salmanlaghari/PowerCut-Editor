...
ExportEngine::ExportEngine():m(std::make_unique<Impl>()){leveldb::Options o;o.create_if_missing=true;leveldb::DB::Open(o,"powercut_render_cache",&m->db);}
...
// ✅ NEW: Watermark overlay function
void ExportEngine::apply_watermark(RGBAFrame* frame) {
  if(m->cfg.remove_watermark) return; // Ad clicked → skip watermark
  // Bottom‑right corner, semi‑transparent white "PowerCut" text area overlay
  int wm_w = 220, wm_h = 60;
  int x = frame->width - wm_w - 24;
  int y = frame->height - wm_h - 24;
  uint8_t alpha = 160; // Semi‑transparent
  for(int py=y; py<y+wm_h && py<frame->height; py++){
    for(int px=x; px<x+wm_w && px<frame->width; px++){
      ...
      pix[0] = (pix[0]*(255-alpha) + 255*alpha)/255; // R
      pix[1] = (pix[1]*(255-alpha) + 255*alpha)/255; // G
      pix[2] = (pix[2]*(255-alpha) + 255*alpha)/255; // B
      pix[3] = 255; // A
    }
  }
}
...
void ExportEngine::worker(){
  ...
  for(int64_t f=0;f<d.tf&&!d.cancel;++f){
    ...
    char k[64];snprintf(k,sizeof k,"f_%09lld_wm_%d",(long long)f,(int)d.cfg.remove_watermark);std::string cc;
    if(d.db->Get({},k,&cc).ok()){
    ...
    apply_watermark(out); // ✅ Apply watermark before encode
...
bool ExportEngine::setup_enc(){
  auto&d=*m; AVCodec const*cv=nullptr,*ca=nullptr;
    ...
    cv=avcodec_find_encoder_by_name("h264_mediacodec");
    if(!cv)cv=avcodec_find_encoder_by_name("hevc_mediacodec");
  #elif defined(__APPLE__)
    cv=avcodec_find_encoder_by_name("h264_videotoolbox");
    if(!cv)cv=avcodec_find_encoder_by_name("hevc_videotoolbox");
  #elif defined(_WIN32)
    cv=avcodec_find_encoder_by_name("h264_nvenc");
    if(!cv)cv=avcodec_find_encoder_by_name("h264_amf");
  #endif
  ...
  av_opt_set(d.vc->priv_data,"preset","veryfast",0);
  av_opt_set(d.vc->priv_data,"tune","zerolatency",0);
  ...
  swr_alloc_set_opts2(&d.swr,&d.ac->ch_layout,d.ac->sample_fmt,d.ac->sample_rate,nullptr,AV_SAMPLE_FMT_S16,44100,0,nullptr);
  swr_init(d.swr);
  return true;
}
